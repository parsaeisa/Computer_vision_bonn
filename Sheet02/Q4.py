...
def task4():
    ...
    print("Comparing and printing mean absolute difference at each level. Please Wait...")
    for i in range(4):
        diff = np.mean(np.abs(cv_pyramid[i] - my_pyramid[i]))
        print(f"Mean absolute difference at level {i}: {diff}")

    print("Calculating the time needed for template matching without the pyramid.  Please Wait...")
    start_time = time.time()
